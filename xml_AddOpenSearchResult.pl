...
open(PTM_DATABASE,"sub_ptmlist_regular.txt"); # slightly modified database of uniprot ptms#####################  PTM FILE              ########################
...
my $xml_deref='new_uniprot.xml';#####################              MAIN OUTPUT FILE        ########################
...
my $morpheus_PSMs_file='PSMs.tsv';#####################              MORPHEUS RESULT FILE              ########################
...
while (<MORPHEUS_RESULTS>)
              ...
              if (($indiv_peptide_ID_data[26])&&($indiv_peptide_ID_data[30]<=100)&&(grep(!/DECOY/, $indiv_peptide_ID_data[13]))) #checks if Target=TRUE and Q value <= 100.0 and Not DECOY
                             {
                             ...
                             foreach my $mass_shift (keys %PTM_mass_hash)
                                           {
                                           my $low = $mass_shift-0.02;
                                           my $high = $mass_shift+0.02;   
